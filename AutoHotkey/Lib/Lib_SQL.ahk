...
Class SQL
{
	...
	static AddDefaultsToSelect()
	{
		...
		;strRegexNeedle := "iO)((fstr|flng|fi64|fint|fcur|fbln|fdtm)\w+)(?!\s+AS\s+)"
		strRegexNeedle := "iO)((\w+\.)?(fstr|flng|fi64|fint|fcur|fbln|fdtm)\w+)"
		...
		While lngFoundPos > 0
		{
			;Here is where we should check if we have a prefix for the table first		
			strBaseName := 
			arrName := strSplit(strMatched.Value(0),".")
			if arrName.MaxIndex() = 1
				strBaseName := arrName[1]
			ELSE
				strBaseName := arrName[2]
			...
			switch subStr(strBaseName,2,3)
			{
				CASE "lng","int","i64","cur":
					...
					strColumns := RegExReplace(strColumns,strRegexNeedle, "0 AS " strMatched.Value(0),,1,strMatched.Pos(0))
				...
				CASE "dtm":
					...
					strColumns := RegExReplace(strColumns,strRegexNeedle, "'9999-12-31' AS " strMatched.Value(0),,1,strMatched.Pos(0))
					
				DEFAULT:
					lngFoundPos += 5 + strMatched.Len(0)
					strColumns := RegExReplace(strColumns,strRegexNeedle, "'' AS " strMatched.Value(0),,1,strMatched.Pos(0))
